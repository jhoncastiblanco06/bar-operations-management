...
interface Usuario {
  ...
  rol: string;
  estado: string;
  sedes?: Sede | null; // Datos de la sede que nos incluye Prisma
}
...
export default function UsuariosPage() {
  ...
  const fetchData = async () => {
    try {
      // Usamos Promise.all para cargar ambas cosas al mismo tiempo y más rápido
      const [resUsuarios, resSedes] = await Promise.all([
        fetch("http://localhost:7086/usuarios"),
        fetch("http://localhost:7086/sedes"),
      ]);
  ...
  const handleCrearUsuario = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:7086/usuarios", {
        method: "POST",
