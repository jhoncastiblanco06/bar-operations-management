...
export default function TerminalCaja() {
  ...
  // 🚀 NUEVOS ESTADOS: Para manejar la Sede
  ...
  useEffect(() => {
    const cargarSedes = async () => {
      try {
        ...
        // Si el usuario logueado tiene sede, la pre-seleccionamos
  ...
  // 2. Cargar las cuentas activas dependiendo de la Sede Seleccionada
  const cargarCuentasAbiertas = async (esSilencioso = false) => {
    if (!sedeSeleccionada) return;
    if (!esSilencioso) setEstaCargando(true);

    try {
      const respuesta = await fetch(
        `${API_URL}/caja/cuentas-activas/${sedeSeleccionada}`,
      );
      if (respuesta.ok) {
  ...
  useEffect(() => {
    ...
    cargarCuentasAbiertas(false); // Carga inicial con animación

    // Refresco automático silencioso cada 10 segundos
    const intervalo = setInterval(() => cargarCuentasAbiertas(true), 10000);
    ...
    try {
      ...
      const respuesta = await fetch(`${API_URL}/caja/cobrar`, {
        ...
        body: JSON.stringify({
          ...
          monto_recibido: metodoPago === "Tarjeta" ? totalCuenta : recibido,
      ...
      if (respuesta.ok) {
        ...
        cargarCuentasAbiertas(true); // Recargamos la lista silenciosamente
      } else {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-2rem)] font-sans">
      {/* 🚀 HEADER INTEGRADO: Selector de Sede */}
      <div className="bg-gray-900 border border-gray-800 rounded-3xl p-4 mb-4 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-lg shrink-0">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-2">
            <span className="text-green-500">Terminal</span>
          </h1>
          <p className="text-xs text-gray-400">Caja y Cobro de Mesas</p>
        </div>
        ...
        <div className="w-full sm:w-72">
          <select
            ...
            onChange={(e) => {
              ...
              setCuentaSeleccionada(null); // Limpiamos la mesa si cambian de sucursal
            }}
            className="w-full bg-gray-950 border border-green-500/30 rounded-xl px-4 py-2.5 text-white outline-none focus:border-green-500 font-bold transition-all shadow-lg shadow-green-500/10 cursor-pointer"
          >
      ...
      {/* CUERPO PRINCIPAL */}
      {!sedeSeleccionada ? (
        <div className="flex-1 flex flex-col items-center justify-center bg-gray-900/30 border border-gray-800 border-dashed rounded-3xl p-6 text-center">
          <span className="text-6xl mb-4 opacity-50">🏪</span>
          <h2 className="text-2xl font-bold text-gray-300">Caja Inactiva</h2>
          <p className="text-gray-500 mt-2 max-w-sm">
            Selecciona la sucursal en el menú superior para ver las cuentas
            abiertas.
          </p>
        </div>
      ) : estaCargando ? (
        <div className="flex-1 flex items-center justify-center text-green-500 font-bold animate-pulse tracking-widest uppercase">
          Cargando terminal de caja...
        </div>
      ) : (
        <div className="flex flex-col lg:flex-row gap-6 flex-1 overflow-hidden pb-10 lg:pb-0">
          ...
          <div className="flex-1 flex flex-col bg-gray-900/50 rounded-3xl border border-gray-800 overflow-hidden shadow-xl">
            <header className="p-6 border-b border-gray-800 bg-gray-950">
              <h2 className="text-xl font-black text-white flex items-center gap-3">
                <span className="bg-gray-800 p-2 rounded-xl border border-gray-700 shadow-inner leading-none text-lg">
                  🖥️
                </span>
                Cuentas Abiertas
              </h2>
            ...
            {!cuentaSeleccionada ? (
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-gray-950/50">
                ...
                <p className="text-sm text-gray-500">
                  Selecciona una mesa del panel izquierdo para iniciar el
                  proceso de cobro.
                </p>
              </div>
            ) : (
              <>
                ...
                {/* Formulario de Pago */}
                <div className="flex-1 p-6 space-y-6 overflow-y-auto bg-gray-900/50 scrollbar-thin scrollbar-thumb-gray-800">
                  {/* Método de Pago */}
                  <div>
